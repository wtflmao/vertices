...
const std::array<std::unique_ptr<Node>, BVH_NODE_CHILDREN>& Node::getChildren() const noexcept {
    return children;
}
...
static std::unique_ptr<Box> computeBox(const std::vector<std::shared_ptr<Triangle>> &faces) {
    if (faces.empty()) {
        return {};
    }
...
void Node::split() {
    ...
    std::ranges::sample(boxedFaces, samples.begin(), std::min(static_cast<size_t>(1000), boxedFaces.size()),
    ...
    // split the box
    const std::size_t steps = boxedFaces.size() / BVH_NODE_CHILDREN;
    ...
    for (std::size_t i = 0; i < BVH_NODE_CHILDREN; i++) {
        if (i != BVH_NODE_CHILDREN - 1) {
            ...
            children[i] = std::make_unique<Node>(std::vector(boxedFaces.begin() + i * steps, boxedFaces.end()), getDepth() + 1);
        }
    }
