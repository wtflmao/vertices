...
Eigen::Matrix4d TransFormsGroup::getTransForm(const std::string &start,
...
Eigen::Matrix4d TransFormsGroup::pushTransForm(const std::string &parent,
...
{
  Eigen::Vector3d positon(x, y, z);
  Eigen::Quaterniond quat(rx, ry, rz, rw);
...
std::string TransFormsGroup::toString()
{
  ...
  for (iter = tfg.begin(); iter != tfg.end(); iter++)
  {
    ...
    for (int i = 0; i < iter->second.size(); i++)
    {
      ...
      // cout <<"\t\tmatrix : ";
      Eigen::VectorXd vxd = TransForms::H2EulerAngle(iter->second[i].t);
...
{
  Eigen::Matrix4d base2target = getTransForm(target, base);
  Eigen::Matrix3d rotate;
  Eigen::Vector3d transpose;
  TransForms::HDecompose(base2target, rotate, transpose);
  for (uint64_t i = 0; i < points.size(); i++)
  {
    Eigen::Vector3d temp_v3d = rotate * points[i];
