...
double computeStdev(const std::vector<Triangle *> &samples, int axis) {
    ...
    double variance = std::transform_reduce(positions.begin(), positions.end(), 0.0, std::plus<>(),
...
void Box::split() {
    ...
    std::sample(boxedFaces.begin(), boxedFaces.end(), samples.begin(), std::min(size_t(1000), boxedFaces.size()),
    ...
    for (std::size_t i = 0; i < BVH_NODE_CHILDREN; i++) {
        if (i != BVH_NODE_CHILDREN - 1) {
            children[i] = computeBox(
                std::vector<Triangle *>(boxedFaces.begin() + i * steps, boxedFaces.begin() + (i + 1) * steps));
        } else {
